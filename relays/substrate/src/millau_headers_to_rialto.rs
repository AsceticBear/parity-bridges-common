...
impl SubstrateHeadersSyncPipeline for MillauHeadersToRialto {
	...
	async fn make_submit_header_transaction(
		...
		log::info!(target: "bridge", "bear(make_submit_header_transaction) - header number {:?}", header.header());
	...
	async fn make_complete_header_transaction(
		...
		log::info!(target: "bridge", "bear(make_complete_header_transaction) - header id {:?}", id.1);
